...
def activate():
    ...
    while(cap.isOpened()):
        # read image
        ret, img = cap.read()
        framefliped = cv2.flip(img,1)
        cv2.imshow('flipped video',framefliped)
        img = framefliped
        ...
        if count_defects == 1:
            cv2.putText(img,"dummy function", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
            print('dummy function')
            #auto.hotkey('winleft','a')
        elif count_defects == 2:
            str = "This is a basic hand gesture recognizer"
            cv2.putText(img, 'close software', (5, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
